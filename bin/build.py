...
# ─── OKLCH (Xcode 27 recipes store colours in OKLCH, not sRGB) ─────────────
...
def oklch(h: str) -> tuple[float, float, float]:
    """sRGB hex -> (lightness, chroma, hue-radians) in Oklab's LCh form."""
    r, g, b = (_srgb_to_linear(c) for c in h2r(h))
    ...
    return L, math.hypot(A, B), math.atan2(B, A) % (2 * math.pi)


def dvt_color(h: str, opacity: float = 1.0) -> dict:
    """A DVTOpaqueColor / DVTColor as Xcode 27's Codable conformance wants it."""
    L, C, H = oklch(h)
    # A grey's hue is whatever rounding error survived the matrices; pin both to
    # zero so the greys stay greys and regenerating stays byte-stable.
    if C < 1e-6:
        C = H = 0.0
    return {
        "hue": {"radians": round(H, 12)},
        "chroma": {"exact": round(C, 12)},
        "lightness": {"exact": round(L, 12)},
        "opacity": opacity,
        "gamut": "sRGB",
    }
...
# Xcode 27 replaced the flat .xccolortheme plist with a *recipe*: two palette
# anchors plus per-role overrides, resolved in OKLCH at display time, and a
# background that may be a gradient. Default+ is hand-tuned rather than derived
# from two hues, so every syntax role is pinned in `colorOverrides` and the
# anchors only steer chrome Xcode generates for itself.
#
# Key names and nesting were read back from Xcode 27's own Codable conformances
# (DVTWorkspaceSavedRecipeFile / DVTWorkspaceThemeRecipe); note `colorOverrides`
# rather than the struct's `overrides` property name — the wrong spelling is
# accepted and silently ignored.
#
# Roles Xcode 27 no longer separates are folded onto the nearest survivor:
# `attribute` has no role of its own, and character/regex share `string`.
XCODE27_ROLES = {
    "plainText": "syntax.plain",
    "comment": "syntax.comment",
    "commentEmphasized": "syntax.comment",
    "string": "syntax.string",
    "keyword": "syntax.keyword",
    "number": "syntax.number",
    "preprocessor": "syntax.macro",
    "link": "syntax.url",
    "memberDeclaration": "syntax.declaration",
    "typeDeclaration": "syntax.declaration_type",
    "projectMember": "syntax.project_identifier",
    "projectType": "syntax.project_identifier",
    "otherMember": "syntax.system_member",
    "otherType": "syntax.system_type",
    "markdown": "syntax.plain",
    "markdownEmphasized": "syntax.plain",
    "accent": "status.info",
    "debugger": "status.success",
    "diffAddition": "syntax.comment",
    "diffDeletion": "syntax.string",
    "invisibles": "base.invisibles",
    "scopeGuides": "base.muted",
    "codeFoldingRibbon": "base.muted",
    "selectedTextBackgroundColor": "base.selection_background",
    "insertionPointColor": "base.cursor",
    "consoleBackground": "base.background",
}


def gen_xcode27(p) -> tuple[Path, str]:
    overrides = {
        role: {"color": dvt_color(ref(p, dotted))}
        for role, dotted in XCODE27_ROLES.items()
    }
    # Xcode 26 stores the current-line highlight at 50% alpha; keep it translucent
    # rather than pre-flattening, since Xcode 27 composites it over the gradient.
    overrides["currentLineHighlight"] = {
        "colorWithOpacity": dvt_color(ref(p, "base.current_line"), 0.5)
    }

    recipe = {
        "palette": {
            "primary": {"custom": dvt_color(ref(p, "syntax.declaration"))},
            "secondary": {"custom": dvt_color(ref(p, "syntax.project_identifier"))},
            "colorOverrides": overrides,
        },
        "colorScheme": "dark",
        "background": {
            "customGradient": {
                "top": dvt_color(ref(p, "xcode27_extended.gradient_top")),
                "bottom": dvt_color(ref(p, "xcode27_extended.gradient_bottom")),
            }
        },
    }
    doc = {"fileVersion": 1, "recipe": recipe}
    path = ROOT / "xcode" / "Default+.xcworkspacecolortheme"
    return path, json.dumps(doc, indent=2, sort_keys=True) + "\n"


GENERATORS = [
    gen_ghostty, gen_iterm, gen_terminal, gen_tig,
    gen_lazygit, gen_herdr, gen_zsh, gen_slack, gen_xcode27,
]
